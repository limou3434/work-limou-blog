...
可以快速使用一下，如果您的 `Caddy` 没有默认启动则可以使用 `caddy run` 手动启动。
...
然后命令行终端就可以使用 `curl localhost:2015` 获取到 `Hello, world!` 的响应文本。

使用 `caddy stop` 就可以停止 `Caddy` 的服务。
...
其中 `Caddy` 提供了命令行工具 `caddy`。

-   `caddy start`：启动 `Caddy` 服务
-   `caddy run`：临时 `Caddy` 服务
-   `caddy stop`：关闭 `Caddy` 服务
...
实际上 `Caddy` 还支持使用 `Caddyfile` 文件来搭建服务，编写好后直接在相同目录下使用 `caddy adapt` 即可。
