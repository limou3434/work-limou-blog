---
title: 002_limou_2024_01_23_linux_网络编程
createTime: 2025/04/09 13:05:16
permalink: /article/o29by2r5/
---
# 网络编程
...
-   **源 IP 地址(Source IP Address)**：这是指发起通信的设备的 `IP` 地址。在网络通信中，或者一个局域网中，每个主机设备都被路由器分配了一个唯一的 `IP` 地址，用于标识其在网络上的位置。

-   **目的 IP 地址(Destination IP Address)**：这是指接收通信的主机设备的 `IP` 地址，可以告诉网络中的路由器或交换机将数据包发送到哪个设备。

### 1.2.端口号

端口全称 **端口号(Port)** 是传输层协议（如 `TCP` 或 `UDP`）中的一个概念，用于标识网络中一台主机上某一个特定的进程或服务。它是一个 `16` 位的整数，取值范围是 `0-65535`。
...
>   [!WARNING]
>
>   注意：实际上端口号和进程 `ID` 很是相似（当然也有区别），主要是为了使网络和进程两个知识领域进行概念解耦...

有了端口号就可以确定一台拥有 `IP` 地址的主机上的某一个网络进程，这样两台主机就有点对点的在两个进程之间进行交互的依据。

如果你有一个使用 `IP` 地址和端口号的通信场景，比如 `QQ` 消息的发送，源 `IP` 地址是发送方的 `IP` 地址，目的 `IP` 地址是接收方的 `IP` 地址，而端口号则用于区分不同的服务或程序，确保消息到达对端中正确的应用程序或进程。

这样，用户主机和服务主机内的通信，也就是客户端进程和服务端进程之间的进程通信，就是网络通信的本质。数据在主机间转发仅仅是基本的手段，更为重要的是将数据交给双方主机中对应的进程，也就是将进程和一台主机的特定端口号来关联。
...
>
>   浏览器出于安全原因，可能会默认会阻止对某些端口的访问。

>   [!IMPORTANT]
>
>   补充：一台电脑上的端口号为了安全考虑会设置防火墙进行保护，要程序能够正常使用端口号，有时就需要开放防火墙的保护，如果是远端服务器，则可以使用以下方法进行打开，待补充...
>
>   如果是本地的 `Windows` 电脑，则可以按以下方法进行打开，待补充... 
...
>   -   **面向字节流**：以后待补充...
>
>   一般实时性不强的应用会优先使用 `TCP`，有些应用还支持用户自定义选择。

## 2.网络字节序

-   内存中的多字节数据相对于内存地址有大小端之分
-   磁盘文件中的多字节数据相对于文件中的偏移地址也有大端小端之分
-   因此网络传输中的数据流，也同样也有大端小端之分，通常采用大端

发送前，发送主机按照大端的形式存储数据，然后发生后续：

1.   发送主机通常将发送缓冲区中的数据按内存地址 **从低位字节到高位字节的顺序发出**
2.   接收主机把从网络上接到的字节依次保存在接收缓冲区中，**直接按内存地址从低到高的顺序保存**

发送端：

| \[内存低地址\]\[高位字节\]\[后发送\] | 字节 | 字节 | \[内存高地址\]\[低位字节\]\[先发送\] |
| ------------------------------------ | ---- | ---- | ------------------------------------ |
| 11④                                  | 22③  | 33②  | 44①                                  |
...
// 函数声明[旧时写法]
// 端口号[主机字节序->网络字节序]
uint16_t htons(uint16_t hostshort); // 将 16 位主机字节序的 port 转换为网络字节序

// 端口号[网络字节序->主机字节序]
uint16_t ntohs(uint16_t netshort); // 将 16 位网络字节序的 port 转换为主机字节序

// 地址[主机字节序->网络字节序]
in_addr_t inet_addr(const char *ip); // 将主机字节序 ip 从点分十进制字符串形式转换为网络字节序
...
如果希望更加现代化一些，则可以使用下述函数：

```cpp
// 函数声明[现代写法]
// 端口号[主机字节序->网络字节序]
uint32_t htonl(uint32_t hostlong); // 将 32 位主机字节序的 port 转换为网络字节序

// 端口号[网络字节序->主机字节序]
uint32_t ntohl(uint32_t netlong); // 将 32 位网络字节序的 port 转换为主机字节序

// 地址[主机字节序->网络字节序]
int inet_pton(int af, const char *src, void *dst) // 将主机字节序 ip 从点分十进制字符串形式转换为网络字节序(支持 ipv6; af 填 AF_INET 表示 IPv4, AF_INET6 表示 IPv; src 是要转换的 IP 地址字符串; dst 是转换后的二进制格式, 通常存储在 struct in_addr(IPv4) 或 struct in6_addr(IPv6) 中)
    
// 地址[网络字节序->主机字节序]
const char *inet_ntop(int af, const void *src, char *dst, socklen_t size) // 将网络字节序的 ip 从二进制形式转换回点分十进制字符串
```

虽然本系列基本都是用第一种较旧方式，但实际编码中我推荐您使用第二种较新的方式。

>   [!IMPORTANT]
>
>   补充：其实 `htonl, ntohl` 中的 `h` 表示 `host`，而 `n` 表示 `network`。而 `inet_pton, inet_ntop` 中的 `n` 是二进制数字，`p` 是指点分十进制。
...
在 `Linux` 下进行套接字编程时，通常只需要对 `IP` 地址和 `port` 进行字节序的转换，而不需要对发送的数据进行转换，因为套接字接口已经处理了发送数据的转换（不过我们完全可以自己封装一个）。

我们来看一下我们接下来的网络编程大致编写思路如下。

```mermaid
flowchart TD
A["客户端发送数据(自动绑定 ip 和 port)"]
B["设置请求远端服务的 ip 和 port"]
C["htons()"]
D["inet_addr()"]
E["数据"]
F["ntohs()"]
G["inet_ntoa()"]
H["得到请求的 ip 和的 port"]
I["服务端处理数据(手动绑定 ip 和的 port)"]
J["处理数据"]
K["根据之前获取到的客户端 ip 和 port 返回响应"]

A --> B -->|"从主机字节序"| C & D -->|"转化为网络字节序"| E -->|"从网络字节序"| F & G  -->|"转化为主机字节序"| H --> I --> J --> K
```
...
1.   **原始 socket(Raw Socket)**：通常指的是在网络层（`OSI` 模型的第三层）直接操作数据包的一种套接字。使用原始套接字。这需要特殊的权限，程序可以发送和接收没有被深度封装的数据包（但是还是有一些封装的），而不被上层协议栈处理，这使得程序可以更底层地控制网络数据的处理（但是这样的操作可能对网络和系统造成风险）。
2.   **域间 socket(Local Socket)**：常见于本地通信，和命名管道通信很是类似。通过文件系统中的特殊文件来实现通信，通常被用于实现进程间的 `IPC`（进程间通信）。在 `Unix/Linux` 系统中，这种套接字通常是基于文件系统中的文件路径进行通信的，比如基于文件名的命名套接字和抽象命名套接字，因此不需要经过网络协议栈，有更高的本地通信效率。
3.   **网络 socket(Network Socket)**：通常用于描述一般的网络编程中的套接字，也就是传输层以上的编程（`OSI` 模型的第二层）。常见的网络 `socket` 包括 `TCP socket` 和 `UDP socket`，它们分别基于 `TCP` 和 `UDP` 传输协议，是应用层与传输层之间的接口。

上述三种应用场景，理论上应该是有三种接口，但是 `Linxu` 上统一使用同种接口（只是在传递参数的时候会根据不同的宏和结构体进行不同的区分），而我们重点学习最后一种网络套接字（也是我们最常见的一种）。
...
// 成功的时候返回一个套接字描述符(返回值类似文件描述符, 由于 UDP 是面向数据报的, 有专门的根据描述符来使用的读写接口。但是 TCP 中直接可以像文件一样使用，这是因为文件和 TCP 都是面向字节流的), 失败返回负值并且设置 errno. 值得注意的是，可以使用 close() 来关闭这个描述符
...
// 1) domain(协议家族/地址族/域): 指明将来创建的套接字类型, 常见类型如下
    ...
    // c) AF_INET 和 AF_INET6
        // 用于 IPv4/IPv6 网络通信
        // 常用于 TCP 和 UDP 套接字编程
    // d) 其他 AF_NETLINK(用于 Linux 内核与用户空间之间的通信)AF_25, AF_AX25, AF_ATMPVC, AF_APPLETALK, AF_PACKET, AF_UNSPEC(未指定地址族，由系统自动选择合适的地址族), AF_BLUETOOTH(用于蓝牙通信)...
// 2) type(通信类型):
    // a) SOCK_DGRAM: UDP 是面向数据报的, 无需建立连接, 应使用这个
    // b) SOCK_STREAM: TCP 是面向字节流的, 需要建立连接, 应使用这个
    // c) ...

// 3) protocol(协议类别): 选择协议, 基本上前面两个参数填好了这个就固定了, 通常填 0 即可自动推导选择相应的协议
...
// bind() 可以将用户指定的 ip 和 port 在内核中进行强关联, 成功返回 0, 失败返回负值并且设置 error

// 1) socket: 直接填入之前获取到的套接字标识符
// 2) address: 可以填上强转后的套接字结构体, 例如
    // struct sockaddr{/*...*/} 变量, 最原始通用抽象结构
    // sockaddr_in{/*...*/} 变量, 内部包含 IP 和 PORT
    // sockaddr_in6{/*...*/} 变量, 内部包含 IP 和 PORT
    // sockaddr_un{/*...*/} 变量, 通常包含本地文件系统中的路径
// 3) address_len: 传入的对象结构体的字节长度(其实就是 sizeof())

```

`套接字 API` 是一层抽象的网络编程通用接口，但是各种套接字编程类型的原理并不相同，因此其中 `bind()` 的参数 `const struct sockaddr* address` 的结构有如下三种主要结构：

![image-20240209223429854](../操作系统原理/assets/image-20240209223429854.png)

1.  **`struct sockaddr`**：通用的抽象结构，最原始的结构
2.  **`struct sockaddr_un`**：是用于 `UNIX` 域套接字的结构体
3.  **`struct sockaddr_in`**：用于 `IPv4` 网络地址的结构体（我们重点学习这个）

上述结构体中，接口根据地址类型来判断是哪一种 `socket` 通信，因此使用 `binf()` 的时候需要把三种类型统一强制转化为 `(struct sockaddr*)` 类型。
...
读写相关的接口在 `UDP` 和 `TCP` 两种协议上有些许不同，并且 `TCP` 的读写步骤会较多（侧面反映 `TCP` 要复杂一些）。
...
// 1) sockfd: 套接字标识符
// 2) buf 和 len: buf 指向用于读取的缓冲区, len 是该读取缓冲区的大小(读取缓冲区由程序员来设定)
// 3) flag: 表示调用操作的可选标志，常用的标志有 MSG_CONFIRM, MSG_DONTWAIT, MSG_ERRQUEUE 等, 不用则置 0 即可
...
// 3) flag: 表示调用操作的可选标志, 常用的标志有 MSG_CONFIRM, MSG_DONTWAIT, MSG_ERRQUEUE 等, 不用则置为 0 即可
...
// 该函数最重要的是返回值, 会返回一个提供 IO 服务的套接字, 失败返回 -1, 并且设置 error
...
// 2) addr: 结构体包含了服务端的 ip 和 port 等连接信息
...
在网络通信中，一般都是客户端主动访问服务端，而不是服务端主动连接客户端，这种基于客户端的模式简称 `CS` 模型，我接下来将会编写几个相关的程序供您深入理解 `CS` 模型和套接字编程的相关接口。

>   [!WARNING]
>
>   警告：接下来的代码部分比较多，您要有心理准备...
...
![image-20240314000339847](../操作系统原理/assets/image-20240314000339847.png)

>   补充：关于 `popen()` 的 `type` 的用起来其实不奇怪，就像打开一个文件一样来使用即可，参数 `type` 指定了管道的类型，有两种可能的取值：`"r"` 和 `"w"`。
>
>   *   如果 `type` 是 `"r"`，则 `popen` 打开的是一个用于读取输出的管道。这意味着你可以从命令的标准输出中读取数据。
>   *   如果 `type` 是 `"w"`，则 `popen` 打开的是一个用于写入输入的管道。这意味着你可以将数据写入到命令的标准输入中。
>
...
>   注意：当然目前的服务端还是不支持类似 `vim` 和 `top` 等指令，原因是这些指令都需要进行交互，会比较麻烦一些。
...
read_and_write["read:read()、recv()/write:write()、send()"]
```
...
我先不进行读写操作，先把 `TCP` 的链接建立起来，还有一些初始化的操作。
...
public:
    ...
    void Start()
    {
        ...
        while (true)
        {
            ...
            //3.关闭服务套接字(多线程版本不能关闭, 否则会导致服务端被异常关闭, 这也很好理解, 这里不是多进程, 主线程关闭服务套接字会导致共享的子线程读写套接字时出现错误, 因此让子线程自己关闭即可)
            //close(serviceSock);
        }
    }
...
上述的线程池代码还有一些缺陷，只要客户端不退出，就会一直进行读写，让子线程成为一个长链接，这种情况一般是不常见的。但是这个问题其实还好，后续有机会我将改进个问题（如果我记得的话...）。

当然，线程池带来的上限也有好处，不会导致服务器压力过大导致出现资源短缺的问题。

##### 3.3.2.7.加入实际项目

可以尝试把代码改成翻译软件、多人贪吃蛇、三子棋小游戏，甚至封装得好一些的话，联合 `UDP/TCP` 来开发一个小项目，让用户可以选择使用 `TCP` 和 `UDP` 两种模式进行链接。

这里我修改为转化大小写的小项目，只需要修改上述的 `tcp_server.hpp` 和 `tcp_client.cpp` 即可。

```cpp
//tcp_server.hpp(加入实际项目)
...
到此，您具备了 `C++` 网络服务器并发开发的基本能力。实际上，我们上面学习的都是应用层偏上接口的应用，但是会涉及到一些传输层中 ` UDP` 协议和 `TCP` 协议的知识，两者具体的区别在代码中是可以看出来一些的（前者不需要进行链接和监听，后者则需要进行链接和监听）。这里提前先简单归纳一下，知道一些事实即可，后面提及协议的时候，我还会继续细化。

1.   `UDP` 是面向无链接和数据报的不可靠通信协议，出现数据丢包时不会重传数据包，但凭借现在的网络环境我们很难测试出来这一现象。其中无链接我们在代码中就可以看到（没有像 `TCP` 一样使用 `listen()` 和 `accept()` 接口），而数据报是指“发一条消息得到一条完整消息”。注意“不可靠”只是一个中性词，不可靠带来的一个显著好处就是代码编写容易，不会像 `TCP` 一样编写过于复杂的代码。

2.   `TCP` 是面向有连接和字节流的可靠性通信协议，出现数据丢包时会重新传数据包。其中字节流是指“只有到读的时候才把数据一次读走”。提到 `TCP` 协议基本离不开“三次握手”和“四次挥手”，我这里只告诉您这两个过程发生的时间点。

     **(1)三次握手**：在内部会自动进行“三次握手”来建立连接，这个过程是客户端调用 `connect()` 后发起的，但 `accept()` 本身并不参与“三次握手”，该过程由操作系统来自动完成，只是“三次握手”结束后，`accept()` 才把建立好的类似句柄的资源从内核中返回给服务端用户层，供服务端的用户读写使用（也就是返回一个文件描述符 `fd`），而通过这个文件标识符就可以进行 `IO` 操作。

     **(2)四次挥手**：而两个端都使用 `close(fd)` 断开连接时，就会释放连接资源，会导致“四次挥手”的行为，该行为需要双方共同协作，而不是仅一方主动，而该过程也是由操作系统保证的。

![1715680189880](../操作系统原理/assets/1715680189880-1715680231542-4.jpg)

当然，我们下一节之后的内容，我将会从这节套接字编程的编码实践基础上，从应用层一直讲解到应用层、传输层、网络、数据链路层的常见协议，详细讲解每一层的某一具体协议的报头部分结构。

>   强调：您一定要做到可以自己编写出上述相关的套接字代码，最起码也要看得懂。
