...
简单提及一下，`minilube`、`kubeadm`、`kind` 可以提供 `k8s` 集群环境（相当于集群脚手架），而 `kubectl` 负责操作和管理该 `k8s` 集群的资源，是三个不同脚手架的底层工具。

-   **Minikube**：通常用于单节点学习环境
-   **Kind**：通常用于多节点测试环境
-   **Kubeadm**：通常用于实际的生产环境
...
在 `K8s` 中，**资源清单** 指的是用于描述 `k8s` 集群中各类资源的声明性配置文件，通常以 `YAML` 或 `JSON` 格式编写。资源清单定义了集群中工作负载（如何运行、如何进行网络配置、如何管理存储和资源调度等。常见的资源清单类型：
...
# gcr.io/k8s-minikube/kicbas 这个基础镜像内部包含了运行 k8s 的基础工具), 适用于 Minikube 的 KIC 功能(K8s In Docker, 一个在 Docker 容器中运行 K8s 的方法), minikube start 还会启动这个镜像为容器, 方便开发者进行集群部署, 您可以使用 sudo docker container ls 进行查看
...
K8s control plane is running at https://192.168.49.2:8443 # 这行表示 K8s 控制平面(其实就是之前架构图中提到的 k8s 核心组件)正在运行，并且暴露在 https://192.168.49.2:8443 地址上(这个 ip 地址是在虚拟网卡那得到的, 注意这个地址很类似一个路由地址, 因此可以理解为宿主主机为集群的路由器, 集群的所有请求都需要通过路由器来请求)
...
>   2.  **66d457cb86**：这是 `ReplicaSet` 控制器的标识符。`K8s` 会为每个 `Deployment` 创建一个 `ReplicaSet` 来确保指定数量的 `Pod` 副本在集群中运行。这个标识符是由 `K8s` 自动生成的
...
> - `ExternalName` 用于将 `K8s` 服务代理到外部 `DNS`（以后了解）
>
...
>   
>   - **Pod 和服务发现 IP** 位于 `K8s` 集群内部的私有子网。每个 `Pod` 都有自己的 `IP` 地址，并且这些 `IP` 地址在 `K8s` 的内部网络中进行路由。
>   - **Minikube Node IP** 作为宿主机的 `IP` 地址，与 `K8s` 内部的网络（`Pod` 和服务发现 `IP`）在网络层次上是分离的，通常作为出口或入口点提供与外界的通信。
...
>
> 在生产环境的 `K8s` 集群中，`NodePort` 和 `LoadBalancer` 之间的区别更加明显。简单来说，`NodePort` 只能通过宿主主机来访问，适用于在宿主主机内或通过特定的外部访问场景。`LoadBalancer` 在云平台中能够实现真正的外部访问。
...
# 查看 minikube 的配置
$ kubectl config view
apiVersion: v1  # 配置文件的 API 版本，表示该配置符合 K8s v1 版本的规范
clusters:  # 定义了集群的配置
- cluster:  # 集群配置的具体内容
    ...
    server: https://192.168.49.2:8443  # K8s API Server 的地址，格式为协议 + IP 地址 + 端口（此处为 Minikube 的默认 API 地址）
...
kind: Config  # 配置文件的类型，表示这是一个 K8s 配置文件
...
| csi-hostpath-driver         | minikube | disabled     | K8s                     |
| dashboard                   | minikube | disabled     | K8s                     |
| default-storageclass        | minikube | enabled ✅   | K8s                     |
...
| metrics-server              | minikube | disabled     | K8s                     |
...
| volumesnapshots             | minikube | disabled     | K8s                     |
...
>   ```
>
>   ![image-20241107192334667](./assets/image-20241107192334667.png)
>
...
>
> 而所谓的 `K8s` 控制平面组件，就是指内嵌到 `minikube node` 中的核心 `k8s` 组件集合。
...
// 连接 MongoDB
const mongoUrl = 'mongodb://mongodb:27017'; // MongoDB 服务名称在 K8s 内部的 DNS 解析是 mongodb
let db;
...
在 K8s 中创建 MongoDB 部署和服务。这里我们假设使用一个主数据库，模拟分布式数据库架构。
...
不要让容器系统管理端口号。在Borg 机器上运
行的所有容器都共享主机的 IP 地址，因此 Borg
会在调度过程中为容器分配唯一的端口号。当容
器迁移到新机器上时（有时在同台机器上重启时
也是如此），它会获得一个新的端口号。这意味
着诸如 DNS（域名系统）之类的传统网络服务必
须被自定义服务所取代，以适应这种端口号的动
态分配机制。
